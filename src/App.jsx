      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', position: 'relative' }}>
        ...
        <div className={`slfa-left${showChat ? ' slfa-left--hidden' : ''}`}>
          {showProfile
            ...
            ? <RankingPage />
            : tab === 'perfil'
            ? <PerfilPage key={perfilInitialTab} onClose={() => { setTab('chats'); setPerfilInitialTab('menu') }} initialTab={perfilInitialTab} onOpenSupport={() => { setTab('soporte-user'); setShowProfile(false) }} onGoPanel={() => { setTab('admin'); setShowProfile(false) }} />
            : tab === 'llamadas'
            ? <LlamadasPage />
            : tab === 'estados'
            ? <EstadosPage />
