...
exports.CeroEngine = exports.COLORS = void 0;
...
// Constantes
// ─────────────────────────────────────────────────────────────────────────────
exports.COLORS = ['magenta', 'gold', 'blue', 'green'];
...
function createDeck() {
    let id = 0;
    const c = (color, value, type) => ({ id: id++, color, value, type });
    const deck = [];
    for (const col of exports.COLORS) {
        deck.push(c(col, '0', 'number'));
        for (let n = 1; n <= 9; n++) {
            deck.push(c(col, String(n), 'number'));
            deck.push(c(col, String(n), 'number'));
        }
...
// Reglas
// ─────────────────────────────────────────────────────────────────────────────
function activeColor(top, chosen) {
 ...
 */
function blockReason(card, top, chosen, stack, hand) {
    if (!top)
        return isWild(card) ? 'No podés empezar con comodín' : null;
    if (stack > 0) {
...
function canPlayCard(card, top, chosen, stack, hand) {
    return blockReason(card, top, chosen, stack, hand) === null;
}
function nextIdx(cur, dir, n) {
    return (cur + dir + n * 10) % n;
}
function applyEffect(ctx, card, newSize) {
...
function applyColorChoice(ctx, color) {
...
class CeroEngine {
    ...
    get topDiscard() { return this._discard[this._discard.length - 1] ?? null; }
    get deckLeft() { return this._deck.length; }
    // ── deal() ─────────────────────────────────────────────────────────────────
    deal() {
        if (this._phase !== 'waiting')
            return this._fail('La partida ya fue repartida');
    ...
    play(playerIdx, cardId) {
        if (this._phase === 'game_over')
            return this._fail('La partida terminó');
        if (this._phase === 'color_pick')
            return this._fail('Elegí color primero');
        if (this._phase === 'waiting')
            return this._fail('La partida aún no fue repartida');
        if (playerIdx !== this._current)
            return this._fail('No es tu turno');
        const hand = this._hands[playerIdx];
        const cardIdx = hand.findIndex(c => c.id === cardId);
        if (cardIdx === -1)
            return this._fail('Esa carta no está en tu mano');
        ...
        this._applyPatch(applyEffect({ players: this.players, current: playerIdx, direction: this._direction, drawStack: this._drawStack, pendingTurn: this._pendingTurn }, card, effectSize));
        if (!isWild(card))
            this._chosenColor = card.color;
        return {
            ok: true,
    ...
    draw(playerIdx) {
        if (this._phase === 'game_over')
            return this._fail('La partida terminó');
        if (this._phase === 'waiting')
            return this._fail('Aún no fue repartida');
        if (playerIdx !== this._current)
            return this._fail('No es tu turno');
    ...
    pickColor(playerIdx, color) {
        ...
        if (!exports.COLORS.includes(color))
    ...
    whyBlocked(playerIdx, cardId) {
        if (this._phase === 'game_over')
            return 'La partida terminó';
        if (this._phase === 'color_pick')
            return 'Elegí color primero';
        if (this._phase === 'waiting')
            return 'Partida no iniciada';
        if (playerIdx !== this._current)
    ...
    _applyPatch(p) {
        ...
        if (p.drawStack !== undefined)
            this._drawStack = p.drawStack;
        if (p.winner !== undefined)
            this._winner = p.winner;
        if (p.pendingTurn !== undefined)
...
exports.CeroEngine = CeroEngine;
//# sourceMappingURL=CeroEngine.js.map