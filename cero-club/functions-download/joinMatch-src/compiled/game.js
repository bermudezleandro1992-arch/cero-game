...
async function startMatch(db, matchRef, match) {
    ...
    const engine = new CeroEngine_1.CeroEngine({ playerCount: playerIds.length, names, handSize: CFG.HAND_SIZE });
...
exports.joinMatch = (0, https_1.onCall)({ region: CFG.REGION, timeoutSeconds: 30 }, async (request) => {
    ...
    const { charged, coinsLeft } = await db.runTransaction(async (tx) => {
        ...
        // ── Pago en Coins ───────────────────────────────────────────────────
        guard(balance >= CFG.ENTRY_COST_CC, 'resource-exhausted', `Saldo insuficiente. Necesitás ${CFG.ENTRY_COST_CC} CC, tenés ${balance}.`);
        tx.update(userRef, { ceroCoins: firestore_1.FieldValue.increment(-CFG.ENTRY_COST_CC) });
    ...
    if (requestedMatchId) {
        ...
        for (const docSnap of openSnaps.docs) {
            ...
            if (d.playerIds.includes(uid))
                continue; // no unirse a sala propia
            matchId = docSnap.id;
            playerIndex = d.playerIds.length;
            break;
        }
    ...
    if (matchId) {
        ...
        await db.runTransaction(async (tx) => {
            ...
            guard(!d.playerIds.includes(uid), 'failed-precondition', 'Ya estás en esta sala');
            const newPlayers = [...d.players, { uid, name: request.auth.token.name ?? 'Jugador', index: d.playerIds.length }];
            const newIds = [...d.playerIds, uid];
